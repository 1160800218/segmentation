...
def backward_maximum_matching(dic, in_text):
    ...
    for str in text:
        ...
        while len(str) > 0:
            ...
            # while tryWord not in word_list:             # Trie树：while not trie.contain(tryWord):
                # 若子串长度为1，跳出循环
                if len(tryWord) == 1:
                    break
...
def main():
    seg_text = backward_maximum_matching(dic_path, sent_path)
    # print(seg_text)
    output_2_file(seg_text, seg_BMM_path)
